...
這是一個前後端分離架構的的個人作品集網站，前端使用 Next.js、TypeScript，後端使用 FastAPI、Python 與輕量級的資料庫 SQLite 建立，用來整理個人技術背景、專案經驗與展現數據工程 / 前後端開發的能力。

網站除了呈現作品展示、職涯時間軸、技能整理與教育背景也提供聯絡表單更是證明我擁有將專案以未來容易維護、部署、可持續延伸的規劃能力。
